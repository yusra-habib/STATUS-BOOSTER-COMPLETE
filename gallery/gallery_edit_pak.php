...
//if(isset($_POST['edit_data'])){
    $id=$_GET['edit_id'];
    $postQuery="SELECT * FROM imagepak WHERE idImage='$id'";
    $runPQ=mysqli_query($conn,$postQuery);
    foreach($runPQ as $row){
        ?>
  <h1> UPLOAD IMAGE</h1>
  <form class="form" action="gallery_update_three.php" method="post" enctype="multipart/form-data" autocomplete="off">
