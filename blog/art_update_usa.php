...
if(isset($_POST['update_btn'])){
    ...
    if($fileName!=''){
        ...
        if(in_array($fileActualExt,$allowed)){
            ...
            $fileDestiantion="blog/".$fileName;
            $result1=mysqli_query($conn,"UPDATE blogusa SET blogtitle='$edit_title', bloglink='$edit_link', imageusa='$update_filename' WHERE idblog='$id'")  or die("failed to query database" . mysqli_error($conn));
            move_uploaded_file($fileTempName,$fileDestiantion);
            header("Location:dis_art_usa.php?upload=editsuccess");
        }else{
            $_SESSION['status']="Please upload correct file";
            header('location:art_update_usa.php');
        }
    }else{
